...
for year in range(1995,2015):
   if year < int(proj_start):
      netcdf_present_day_files.append(RCMdir + '/' + \
            RCMver + '/ISMIP6/GrIS/' + CMIP + '-histo_1950_' + histo_end + '/' + RCMver.replace('MAR','MARv') + '-' + monthly_or_yearly + '-' + CMIP + '-histo-{:4d}.nc'.format(year))
   else:
      netcdf_present_day_files.append(RCMdir + '/' + \
            RCMver + '/ISMIP6/GrIS/' + CMIP + '-' + rcp + '_' + proj_start + '_2100/' + RCMver.replace('MAR','MARv') + '-' + monthly_or_yearly + '-' + CMIP + '-' + rcp  + '-{:4d}.nc'.format(year))
...
# Bias correction
if biasCorrection: ##{{{
   outputBiasFilename = '-'.join( (RCMver, CMIP, rcp, basinsRaster, 'runoffBias') ) + '.mat'
...
print(' ')
# Output filename
outputFilename = '-'.join( (RCMver, CMIP, rcp, runoffCalculation.replace('-','_'), basinsRaster) )
