...
load projected_retreat_MARv313.mat 
...
for l=1:7,
    ...
    % CESM2 ssp585
    plot(t,retreat.IPSLCM6ALRe04.ssp585.high(l,:),'color',cols(2,:),'linewidth',lwthick);
    plot(t,retreat.IPSLCM6ALRe04.ssp585.med(l,:),'color',cols(2,:),'linewidth',lwthick);
    plot(t,retreat.IPSLCM6ALRe04.ssp585.low(l,:),'color',cols(2,:),'linewidth',lwthick); 

    plot(t,retreat.IPSLCM6ALRe54.ssp585.high(l,:),'color',cols(2,:),'linewidth',lwthick);
    plot(t,retreat.IPSLCM6ALRe54.ssp585.med(l,:),'color',cols(2,:),'linewidth',lwthick);
    plot(t,retreat.IPSLCM6ALRe54.ssp585.low(l,:),'color',cols(2,:),'linewidth',lwthick); 

    xlim([2015 2300]); ylim([-300 5]);
...
dy = 0.04;
fs0 = 4;
annotation('textbox','position',[tx,ty+2*dy,0,0],'string','IPSLCM6ALRe04-ssp585','fontsize',fs0,'interpreter','latex','edgecolor','none','color',cols(1,:));
annotation('textbox','position',[tx,ty+2*dy,0,0],'string','IPSLCM6ALRe54-ssp585','fontsize',fs0,'interpreter','latex','edgecolor','none','color',cols(2,:));

saveplot(17,8,300,'retreat_projections_MAR313.png');
close all;
...
for l=1:7,
    ...
    axes(a(l)); hold on;
    plot(t,retreat.IPSLCM6ALRe04.ssp585.TF(l,:),'color',cols(1,:),'linewidth',lwthick);
    plot(t,retreat.IPSLCM6ALRe54.ssp585.TF(l,:),'color',cols(2,:),'linewidth',lwthick);

    xlim([2015 2300]); ylim([1 11]);
    set(gca,'fontsize',fs,'box','on');
...
dy = 0.03;
fs = 5;
annotation('textbox','position',[tx,ty-2*dy,0,0],'string','IPSLCM6ALRe04-ssp585','fontsize',fs,'interpreter','latex','edgecolor','none','color',cols(1,:));
annotation('textbox','position',[tx,ty-2*dy,0,0],'string','IPSLCM6ALRe54-ssp585','fontsize',fs,'interpreter','latex','edgecolor','none','color',cols(2,:));

saveplot(17,8,300,'TF_projections_MAR313.png');
close all;
