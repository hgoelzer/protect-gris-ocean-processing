...
lat=ncread(Tfiletoload1,'lat');
z=ncread(Tfiletoload1,'depth');
%time=ncread(Tfiletoload1,'time');
lon_vert=ncread(Tfiletoload1,'lon_bnds');
lat_vert=ncread(Tfiletoload1,'lat_bnds');

% read only TS between 200 and 500 m and one point either side
depthinds = [max(find(z<200)):min(find(z>500))];
z = z(depthinds);
T1=ncread(Tfiletoload1,'thetao',[1,1,depthinds(1),1],[Inf,Inf,length(depthinds),Inf]);
S1=ncread(Sfiletoload1,'so',[1,1,depthinds(1),1],[Inf,Inf,length(depthinds),Inf]);
T2=ncread(Tfiletoload2,'thetao',[1,1,depthinds(1),1],[Inf,Inf,length(depthinds),Inf]);
S2=ncread(Sfiletoload2,'so',[1,1,depthinds(1),1],[Inf,Inf,length(depthinds),Inf]);
...
% get coords of MPIESM points on BedMachine grid
% gridcell vertices
[x_vert,y_vert] = latlon2utm(lat_vert,lon_vert);
% gridcell centres
[x,y] = latlon2utm(lat,lon);
...
basin = NaN(size(Xreg,1),size(Xreg,2));
for k=1:7, % since there are 7 basins
    regions(k).MPIESM12HRinds = find(inpolygon(Xreg,Yreg,regions(k).ocean.x,regions(k).ocean.y));
    basin(regions(k).MPIESM12HRinds) = k;
end
...
for jj=1:7,
    for kk=1:length(z),
        TF_basins_z(jj,kk,:) = nanmean(TFreg_vec(regions(jj).MPIESM12HRinds,kk,:),1);
    end
end
